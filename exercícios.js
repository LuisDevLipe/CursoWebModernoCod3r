...
console.log(inverter({ nome: "Luis", profissao: "Programador" }))
...
function filtrarPorQuantidadeDeDigitos(array, limitador) {
  limitador = Number(limitador)
  const newArray = array.filter(function (numero) {
    numero = String(numero)
    if (numero.length === limitador) {
      return numero
    }
  })
  return newArray
...
// Crie uma função que recebe um array de números e retonar o segundo maior número presente nesse array.

function segundoMaior(array) {
  return quickSort(array)[array.length - 2]
}
...
// As notas de cada estudante estarão num array, conforme exemplo abaixo. Você deverá calcular a média da nota de cada aluno e retornar um objeto com os atributos nome e media , que indica o aluno que teve o melhor desempenho nas notas

function receberMelhorEstudante(object) {
  function calcularMedia(array) {
    return array.reduce((acc, total) => acc + total) / array.length
  }
  let array_de_notas = []
  for (const key in object) {
    if (Object.hasOwnProperty.call(object, key)) {
      array_de_notas.push({ nome: key, media: calcularMedia(object[key]) })
    }
  }
  array_de_notas.sort((a, b) => a.media - b.media)
  return array_de_notas[array_de_notas.length - 1]
}
